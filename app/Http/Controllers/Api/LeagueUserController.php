...
class LeagueUserController extends Controller
{
    ...
    {
        $league_user = LeagueUser::find($id, ['id_user', 'email', 'password16']);
        if (!$league_user) {
            return [false, false];
        }
    ...
    {
        $league_user = LeagueUser::where('email', $email)->first(['id_user', 'email', 'password16']);
        if (!$league_user) {
            return [false, false];
        }
        return [$league_user, response()->json($league_user)];
    ...
    public static function isPasswordValid($credentials)
    {
        ...
        return self::comparePassword($credentials['password'], $league_user_object->password16);
    }
    ...
    public static function insertEmailPassword($credentials)
    {
        ...
        $league_user_object->password16 = $credentials['password'];
